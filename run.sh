...
old_domain=$1
new_domain=$2
slug=${3:-public_html}  # Default to 'public_html' if not provided

# Define the old domain path
domain_old_path="/home/$old_domain/$slug"
# Define the new domain path
domain_new_path="/home/$new_domain/$slug"
...
backup_domain() {
    ...
    if ! wp --allow-root plugin is-active all-in-one-wp-migration; then
        ...
        sudo chown -R "$owner_group" /home/"$domain"/"$slug"/wp-content/plugins/all-in-one-wp-migration/
        sudo chmod -R 755 /home/"$domain"/"$slug"/wp-content/plugins/all-in-one-wp-migration/
    else
        wp --allow-root plugin update all-in-one-wp-migration
        echo "all-in-one-wp-migration is already active"
    fi

    local ext_dir="/home/$domain/$slug/wp-content/plugins/all-in-one-wp-migration-url-extension/"
    ...
    backup_dir="/home/$domain/$slug/wp-content/ai1wm-backups"
    # remove older backup
...
restore_domain() {
    ...
    local ai1wm_dir="/home/$domain/$slug/wp-content/ai1wm-backups"
